...
describe('UserService', () => {
    ...
    it('throws ForbiddenException when role is CUSTOMER', async () => {
      await expect(
        service.updateSellerProfile(
          'user-id',
          UserRole.CUSTOMER,
          sellerProfileDto as any
        )
      ).rejects.toThrow(ForbiddenException)
    })

    it('upserts seller profile for SELLER role', async () => {
      mockPrisma.sellerProfile.upsert.mockResolvedValue({
        id: 'sp-1',
        ...sellerProfileDto
      })
      mockRedis.del.mockResolvedValue(1)

      const result: any = await service.updateSellerProfile(
        'user-id',
        UserRole.SELLER,
        sellerProfileDto as any
      )
  ...
  describe('updateCustomerProfile', () => {
    ...
    it('upserts customer profile for CUSTOMER role', async () => {
      ...
      const result: any = await service.updateCustomerProfile(
        'user-id',
        UserRole.CUSTOMER,
        customerProfileDto as any
      )
